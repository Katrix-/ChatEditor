...
class EditorConfigLoader(dir: Path)(implicit plugin: EditorPlugin) extends ConfigLoader[EditorConfig](dir) {
	...
	override protected val default = new EditorConfig {
		...
		override val text    = new Texts {
			...
			override val commandErrorParseBoolean = ConfigValue(
				t"Failed to parse boolean",
				"The message shown when failing to parse a boolean",
				Seq("cmd", "error", "parseBoolean")
			)
			override val commandErrorInvalidPath  = ConfigValue(
				t"Failed to parse path",
				"The message shown when failing to parse a path",
				Seq("cmd", "error", "parsePath")
			)
			...
			override val eCmdAddLine = ConfigValue(
				t"${GREEN}Added new line",
				"The message shown when a player uses the !addLine command",
				Seq("eCmd", "addLine")
			)
			...
			override val eCmdCopy                = ConfigValue(
				t"${GREEN}Copied the selected text",
				"The message shown when a player copies the text.",
				Seq("eCmd", "copy")
			)
			override val eCmdCut                 = ConfigValue(
				t"${GREEN}Cut the selected text",
				"The message shown when a player cuts the text.",
				Seq("eCmd", "cut")
			)
			override val eCmdPaste               = ConfigValue(
				t"${GREEN}Pasted the text on the clipboard",
				"The message shown when a player pastes in text.",
				Seq("eCmd", "paste")
			)
			...
			override val eCmdLintSuccess = ConfigValue(
				t"${GREEN}All good!",
				"The message shown when a player lints a text and there is no problem.",
				Seq("eCmd", "lint")
			)
			...
			override val fileNotOpen        = ConfigValue(
				t"${GREEN}File not open",
				"The message shown when trying to do file stuff, and a file is not open",
				Seq("file", "notOpen"))
			override val fileSaved          = ConfigValue(
				t"${GREEN}File saved.",
				"The message shown when saving a file",
				Seq("file", "save"))
			override val fileReloaded       = ConfigValue(
				t"${GREEN}File reloaded",
				"The message shown when reloading a file",
				Seq("file", "reload"))
		}
	}
}
